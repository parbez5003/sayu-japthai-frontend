  ...
  const navItems = isAdmin
    ? [
        ...
        },
        {
          ...
          name: "Add Product",
          icon: <MdOutlineAddCircle size={23} />,
        },
        {
  ...
  return (
    <div>
      <div
        ...
        className={`lg:hidden fixed inset-0 z-[998] bg-black/50 ${
          open ? "block" : "hidden"
        }`}
      ></div>
      ...
      >
        <div className="flex items-center justify-between gap-2.5 font-medium border-b py-3 border-slate-400 mx-3">
          <div className=" ">
            ...
            <Link to="/" className="text-xl fonts-normal">
              Sayu Japthai
            </Link>
          </div>
          ...
          <ul className="px-2.5 py-5 flex flex-col gap-1 font-medium overflow-x-hidden md:h-[68%] h-[70%] ">
            {navItems?.map((item) => (
              ...
              >
                <Link
                  to={item.path}
                  className=" p-2 flex fonts-normal  rounded gap-2 items-center cursor-pointer duration-300 "
                >
                  {item.icon}
                  {item?.name}
                </Link>
            ...
            <div className="flex-1 text-sm z-50 max-h-60 my-auto font-medium border-t border-gray-400 p-4">
              <ul>
                ...
                <li
                  ...
                  className="p-2.5 flex rounded-md gap-2 items-center cursor-pointer duration-300 font-medium text-[#FFA500]"
                >
                  <MdLogout size={23} />
                  Logout
                </li>
      ...
      <div className="flex justify-between items-center py-1  px-1 md:py-2 md:px-3">
        <div
          className="p-1 text-gray-800 lg:hidden"
          ...
          <div className="relative w-10 h-10 lg:hidden flex-shrink-0">
            ...
            {currentUser?.profilePhoto ? (
              <img
                ...
                alt=""
                className="cursor-pointer w-10 h-10 border-2 border-white rounded-full dark:border-gray-800"
              />
            ) : (
              <img
                src="https://cdn.pixabay.com/photo/2020/07/01/12/58/icon-5359553_640.png"
                alt="Profile"
                className="w-10 h-10 border-2 border-white rounded-full dark:border-gray-800" // Added similar styling
              />
            )}
          </div>
        </div>

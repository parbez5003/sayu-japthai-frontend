...
const UserDashboardState = () => {
    ...
    const totalOrders = orderData?.reduce((acc, order) => acc + (order.foods?.length || 0), 0);
    ...
    const totalBuyAmount = orderData?.reduce((acc, order) => acc + order.total_amount, 0);
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 px-2">
                <div className="bg-gray-100 shadow-lg rounded-md flex items-center justify-between py-6 px-3 text-gray-700 font-medium group">
                    ...
                    <div className="text-right">
                        <p className="text-2xl flex items-center gap-1">  {totalBuyAmount || 0} <FaEuroSign />  </p>
                        <p>Total Buy </p>
                    </div>
                </div>

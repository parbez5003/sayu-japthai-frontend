...
export default function CheckoutForm({ formData }) {
  ...
  const { myCarts, price, quantity, myCartRefetch } =
    useGetMyCarts();
  ...
  const handleCashOnDelivery = async () => {
    try {
      ...
      if (!Array.isArray(myCarts) || myCarts.length === 0) {
        throw new Error(
          "Cart data is invalid. It should be a non-empty array."
        );
      }
      ...
      if (res?.data) {
        ...
        toast.success(
          ` ${total_amount} $ Cash on Delivery order placed successfully.`
        );
      }
    ...
    <>
      <div className="md:flex items-center lg:gap-6 md:gap-6 justify-center lg:h-screen bg-gray-100  mb-10 lg:px-0 md:px-4 px-0 ">
        <form
          ...
          className=" lg:w-6/12 w-full  p-6 bg-gray-50 rounded-md shadow-lg"
        >
        ...
        <div className="  md:border-none  border-t-2 border-gray-600 lg:rounded-lg  bg-gray-50 lg:w-[200px] w-full  lg:h-[200px] h-40  flex items-center justify-center shadow-lg ">
          <button
            ...
            className={` ${
              formData === null
                ? " opacity-60 cursor-not-allowed bg-blue-700  "
                : "bg-blue-700"
            }  px-5 py-3 text-xs font-medium text-center text-white  rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 
                            `}
          >
            Cash On Dalivary
          </button>
